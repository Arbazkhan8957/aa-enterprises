...
async function run() {
  ...
  const generateContactor = (model, current, hpRange) => {
    ...
    return {
      ...
      model: model.replace(' ', ''),
      description: `The Schneider Electric TeSys D ${model} is an industry-leading AC magnetic contactor designed for perfect integration in control systems, specific to heavy motor control and power switching applications. Engineered for compactness, high reliability, and a long mechanical lifespan, it seamlessly handles currents up to ${current}A. It features highly conductive copper coils, advanced interlocking capability, and built-in auxiliary contacts. Safe and easy to install on a standard DIN rail or panel mount.`,
      image: `/images/${id}.png`,
      ...
      features: [
        ...
        `Power Capacity: Suitable for high power motors up to ${hpRange} at 415V`,
  ...
  const newItems = [
    generateContactor('LC1 D95', 95, '45 kW / 60 HP'),
    generateContactor('LC1 D115', 115, '55 kW / 75 HP'),
    generateContactor('LC1 D150', 150, '75 kW / 100 HP')
  ];
  ...
  console.log('Successfully appended Schneider LC1D 95-150 Contactors!');
}
