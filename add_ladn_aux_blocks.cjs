...
async function run() {
  ...
  const categoryName = 'Schneider LADN Auxiliary Blocks'; // Fixed spelling from Auxilary to Auxiliary

  // Ensure category exists
  if (!categories.find(c => c.name === categoryName)) {
    categories.push({
      name: categoryName,
      image: '/images/ladn11.png', // Using LADN11 as the representative image
  ...
  const newItems = [
    generateLADN('LADN02', 0, 2),
    generateLADN('LADN04', 0, 4),
    generateLADN('LADN11', 1, 1),
    generateLADN('LADN13', 1, 3),
    generateLADN('LADN20', 2, 0)
  ];
  ...
  console.log('Successfully appended Schneider LADN Auxiliary Blocks!');
}
